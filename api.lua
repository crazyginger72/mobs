...
function mobs:register_mob(name, def)
	minetest.register_entity(name, {
		...
		on_step = function(self, dtime)
			...
			if self.type == "monster" and minetest.setting_getbool("enable_damage") and self.tamed ~= true then
			...
			elseif self.state == "attack" and self.attack_type == "dogfight" then
				if not self.attack.player or not self.attack.player:is_player() or self.tamed == true then
					self.state = "stand"
					self:set_animation("stand")
					return
				end
...
mobs.spawning_mobs = {}
function mobs:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, higher, next_to)
	mobs.spawning_mobs[name] = true
	if next_to == nil then next_to = {"air"} end
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			if higher==nil then higher = 1 end
			pos1={x=pos.x, y=pos.y+higher, z=pos.z}
			minetest.env:add_entity(pos1, name)
		end
	})
end
